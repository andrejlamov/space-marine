...
(defconst marine-image-ouch-5              (marine-load-image "ouch-5.xpm"))
(defconst marine-image-wicked-2            (marine-load-image "wicked-2.xpm"))
(defconst marine-image-wicked-5            (marine-load-image "wicked-5.xpm"))
...
(defconst marine-list-ouch-5     (list marine-image-ouch-5))
(defconst marine-list-wicked-2   (list marine-image-wicked-2))
(defconst marine-list-wicked-5   (list marine-image-wicked-5))
...
(defun marine-make-scene (type health &optional next-scene)
  (let ((tuple (list type health)))
    (pcase tuple
      (`,'(glance   2) (marine-make-scene-0 marine-list-glance-2   health 'infinite 'marine-get-next-random-frame))
      (`,'(glance   5) (marine-make-scene-0 marine-list-glance-5   health 'infinite 'marine-get-next-random-frame))
      (`,'(attacked 2) (marine-make-scene-0 marine-list-attacked-2 health 'infinite 'marine-get-next-random-frame))
      (`,'(attacked 5) (marine-make-scene-0 marine-list-attacked-5 health 'infinite 'marine-get-next-random-frame))
      (`,'(ouch     2) (marine-make-scene-0 marine-list-ouch-2     health 1         'marine-get-next-same-frame next-scene))
      (`,'(ouch     5) (marine-make-scene-0 marine-list-ouch-5     health 1         'marine-get-next-same-frame next-scene))
      (`,'(wicked   2) (marine-make-scene-0 marine-list-wicked-2   health 1         'marine-get-next-same-frame next-scene))
      (`,'(wicked   5) (marine-make-scene-0 marine-list-wicked-5   health 1         'marine-get-next-same-frame next-scene))
      (_ nil))
    ))
...
(defun marine-on-compilation-finish (buf msg)
  (if (string-match "exited abnormally" msg)
      (let* ((current-health (marine-scene-health marine-current-scene))
             (next-scene (marine-make-scene 'glance 2))
             (ouch       (marine-make-scene 'ouch   current-health next-scene)))
        (marine-reset-current-scene ouch))
    (let* ((current-health (marine-scene-health marine-current-scene))
           (next-scene (marine-make-scene 'glance 5))
